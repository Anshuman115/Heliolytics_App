...
Future<void> runFullSync({
  ...
  try {
    ...
    if (!await band.ensureConnected()) {
      ...
      emit(
        syncSnap(
          ...
          lastErrorMessage:
              ref.read(bandSessionProvider).errorMessage ??
    ...
    void linkLostHandler() => band.onLinkLost();
    link.onLinkLost = linkLostHandler;

    final plan = await resolveSyncWindow(
      ref,
    ...
    if (plan.backendDataThrough != null) {
      ...
      sessionLog.log(
        'Gap to fill: ${gap.inHours}h ${gap.inMinutes.remainder(60)}m',
      );
    }
    ...
    final outcome =
        await SyncFetcher(log: sessionLog.log, store: store, auth: auth).run(
          mac: mac,
          authKey: authKey,
          plan: plan,
          client: link,
          disconnectAfter: false,
          onTypeStart: (code) => emit(
            syncSnap(
  ...
  } on BandLinkDisconnectedException catch (error) {
    sessionLog.log('✗ $error');
    emit(
      syncSnap(
        SessionState.error,
        sessionLog,
        results,
        error: SessionError.bleDisconnected,
        lastErrorMessage:
            'Strap disconnected during sync. Reconnect and retry.',
      ),
    );
  } finally {
    final link = band.session.link;
    if (link != null) link.onLinkLost = null;
