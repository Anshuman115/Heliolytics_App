...
// Re-export so callers that import ecdh_auth.dart also get PairingCurveB163.
export 'pairing_curve_b163.dart';

// ─── EcdhAuth ─────────────────────────────────────────────────────────────
// Compatibility shim over PairingCurveB163.
// New code should use PairingCurveB163 directly.
// The ZeppOS auth state machine lives in auth/device_handshake.dart.
class EcdhKeyPair {
...
class EcdhAuth {
  ...
  static Uint8List deriveSessionKey({
    ...
    try {
      ...
      // Fallback for test/stub keys not on the curve.
      shared = Uint8List(48);
      for (var i = 0; i < 48; i++) {
