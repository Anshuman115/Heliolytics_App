...
const List<String> fetchTypeCodes = [
  ...
  '0x46', // Continuous HR (PPG session, ~1/sec)
...
const Map<String, String> typeCodeLabels = {
  ...
  '0x53': 'probe-53',
  '0x54': 'probe-54',
  '0x55': 'RR intervals — not fetched (~9 MB)',
  '0x56': 'probe-56',
...
/// Build-time defaults — injected via `tool/build_apk.sh` / `--dart-define`.
/// When both are set at build time, release installs seed secure storage on
/// first launch (no manual Settings entry). Leave empty to configure in UI.
const String defaultApiUrl = String.fromEnvironment(
  'API_URL',
  defaultValue: '',
);
const String defaultApiSigningSecret = String.fromEnvironment(
  'API_SIGNING_SECRET',
  defaultValue: '',
);
...
const String defaultStrapAuthKey = String.fromEnvironment(
  'STRAP_AUTH_KEY',
  defaultValue: '',
);
const String defaultStrapMac = String.fromEnvironment(
  'STRAP_MAC',
  defaultValue: '',
);
...
const String strapSetupPendingStorageKey = 'heliolytics.strap_setup_pending';
...
/// Hive box name for the per-day bundle cache (see DailyBundleCacheStorage).
/// Opened once at app startup in main.dart.
const String cachedDaysBoxName = 'cached_days_box';
const String dailyBundleCacheSchemaKey = '__bundle_schema_version__';
const int dailyBundleCacheSchemaVersion = 2;
const String dailyHealthScoresCacheKey =
    'heliolytics.daily_health_scores_cache';
...
/// After the first successful ingest, per-type server coverage controls the
/// fetch window and normal syncs remain incremental.
const int initialSyncBackfillDays = 20;
...
/// Avoid reconnecting and re-uploading on every cold app launch. Manual sync
/// is never throttled and backend coverage remains the fetch authority.
const Duration autoSyncCooldown = Duration(minutes: 15);
const int metricProgressRhrMin = 40;
const int metricProgressRhrMax = 100;
const int metricTrendWeekDays = 7;
const int metricTrendMonthDays = 30;
const int metricTrendSixMonthsDays = 180;
...
/// Standard BLE Heart Rate service + Heart Rate Measurement characteristic.
...
/// ZeppOS encrypted endpoint for live HR control (0x001d).
const int liveHrControlEndpoint = 0x001d;
const List<int> liveHrStartCommand = [0x04, 0x01];
const List<int> liveHrContinueCommand = [0x04, 0x02];
const List<int> liveHrStopCommand = [0x04, 0x00];
const int liveHrKeepaliveIntervalSec = 1;
...
// Tab navigation floats over shell pages, so the final card must scroll clear
// of it instead of ending underneath the overlay.
const double shellContentBottomPadding = 220;
